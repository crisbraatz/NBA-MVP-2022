# -*- coding: utf-8 -*-
"""NBA MVP 2022 Machine Learning

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1Hzi-oVPvb8s8VZW4xgD8AD1tJcR3O1VJ
"""
...
df = pandas.read_csv('/content/drive/MyDrive/df_nba_mvp_1980_2022.csv', sep = ',', decimal = '.')
...
df['MVP Rank'] = numpy.where(df.Season <= 2021, df.groupby('Season')['Pts Won'].rank(ascending = False, method = 'dense'), 0.0)
df['Won MVP'] = numpy.where(df['MVP Rank'] == 1, 1, 0)

df.tail()

df_past = df[df.Season <= 2021]
df_current = df[df.Season == 2022]

corr = df_past.corr(method = 'pearson')
won_mvp_corr = corr[['Won MVP']]
won_mvp_corr['Won MVP'].abs().sort_values()

df_past.Player.count()
...
def plot_two_variables(df, title, var1, var1_order, var2, var2_order):
    ...
    fig.text(x = -0.02, y = -0.08, s = '____________________________________________________', fontsize = 16, color = 'grey', horizontalalignment = 'left', alpha = .2)
...
plot_two_variables(df_past, 'Points Per Game VS Efficiency Rating', 'PTS', 1, 'PER', 1)

df_past[(df_past['Won MVP'] == 1) & (df_past['PTS'] <= 20)][['Player', 'Tm', 'MP', 'PTS', 'TRB', 'AST', 'STL', 'BLK', 'FG%', '3P%', 'FT%', 'WS', 'WS/48', 'Season', 'Rk', 'Overall', 'W/L%']]

plot_two_variables(df_past, 'Team Overall Seed VS Player Value of Replacement', 'Rk', -1, 'VORP', 1)
...
df_past_predicted = []
results_matrix = numpy.zeros(shape = (1, 2021 - 1980, 4, 2))
...
seaborn.heatmap(confusion_matrix(df_past_predicted['Won MVP'], df_past_predicted['Predicted MVP Winner']), annot = True, fmt = '.2f', cmap = 'rocket_r')

matplotlib.pyplot.text(x = -0.02, y = 2.16, s = '__________________________________________', fontsize = 16, color = 'grey', horizontalalignment = 'left', alpha = .2)
matplotlib.pyplot.text(x = -0.02, y = 2.32, s = 'https://crisbraatz.github.io', fontsize = 16, color = 'grey', horizontalalignment = 'left')
matplotlib.pyplot.show()

df_current.Player.count()

df_current = df_current[(df_current['GS%'] > 0.9) & (df_current['GP%'] > 0.7)]
df_current.Player.count()
